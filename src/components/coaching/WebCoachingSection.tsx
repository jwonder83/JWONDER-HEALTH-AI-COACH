      ...
      <div className="grid gap-0 lg:grid-cols-[1fr_min(38%,300px)]">
        <div className="border-b border-neutral-200 p-8 sm:border-b-0 sm:border-r sm:p-10">
          ...
          <button
            ...
            disabled={disabled}
            onClick={run}
            className={`border border-black bg-black px-8 py-2.5 text-[12px] font-semibold uppercase tracking-[0.14em] text-white transition-all duration-200 hover:bg-white hover:text-black active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40 ${omitInnerTitle ? "mt-0" : "mt-8"}`}
          >
          ...
          {loading ? (
            <div className="mt-8 space-y-3 rounded-xl border border-neutral-200 bg-neutral-50 p-5 shadow-inner">
          ...
          {text ? (
            <article className="mt-8 whitespace-pre-wrap rounded-xl border border-neutral-200 bg-white/90 px-5 py-4 text-[17px] font-normal leading-[1.55] tracking-[-0.01em] text-apple-ink shadow-sm">
              {text}
            </article>
          ) : null}
          ...
          {logs.length > 0 ? (
            <div className="mt-10">
              <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-apple-subtle">{historyTitle}</p>
              <ul className="mt-3 max-h-56 space-y-2 overflow-y-auto rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                {logs.map((log) => (
                  <li key={log.id} className="border-b border-neutral-200 pb-2 last:border-0 last:pb-0">
        ...
        <div className="relative min-h-[200px] w-full sm:min-h-[220px] lg:min-h-full">
          ...
          />
          <div
            className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_70%_at_100%_100%,rgba(0,0,0,0.04),transparent)] sm:hidden"
            aria-hidden
          />
        </div>
      </div>
