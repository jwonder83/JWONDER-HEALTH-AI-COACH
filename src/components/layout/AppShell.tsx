...
export function AppShell({ email, showAdminLink, site, children, footerMeta }: Props) {
  return (
    <div className="flex min-h-screen flex-col text-apple-ink dark:text-zinc-100">
      <header className="sticky top-0 z-30 border-b border-neutral-200 bg-white/95 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/95">
      ...
      <footer className="mt-auto border-t border-neutral-200 bg-white px-4 py-12 dark:border-zinc-800 dark:bg-zinc-950 sm:px-6">
        <div className="mx-auto max-w-6xl text-center">
          ...
          {site.copy.footer.secondaryLine.trim() ? (
            <p className="mt-3 text-[13px] leading-relaxed text-apple-subtle dark:text-zinc-400">{site.copy.footer.secondaryLine}</p>
          ) : null}
          <div className="mt-6 flex flex-wrap justify-center gap-x-6 gap-y-2 text-[13px] font-normal">
            {site.copy.footer.links.map((item, i) => (
              <span key={`${item.href}-${i}`} className="contents">
                {i > 0 ? (
                  <span className="text-neutral-300 dark:text-zinc-600" aria-hidden>
                    |
                  </span>
                ) : null}
                <FooterNavLink
                  href={item.href}
                  className="text-apple-ink underline decoration-neutral-300 underline-offset-[6px] transition hover:opacity-60 dark:text-zinc-200 dark:decoration-zinc-600"
                >
                  {item.label}
                </FooterNavLink>
              </span>
