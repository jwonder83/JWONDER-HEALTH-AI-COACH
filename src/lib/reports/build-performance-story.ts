...
export function findWeakMuscleGroups(thisWeekRows: WorkoutRow[], minTotalVolume = 400): WeakMuscleHint[] {
  ...
  for (const id of targets) {
    const p = share(id);
    if (id === "core" && p < 5) {
      hints.push({ id, label: muscleGroupLabel(id), sharePct: Math.round(p * 10) / 10, note: "코어는 그냥… 안 하면 나중에 터져요." });
    } else if (id !== "core" && p < 10) {
      hints.push({
        id,
        label: muscleGroupLabel(id),
        sharePct: Math.round(p * 10) / 10,
        note: "이번 주 기록에서 비중이 좀 밀렸어요.",
      });
    }
  }
...
export function buildPerformanceStoryReport(rows: WorkoutRow[], now = new Date()): PerformanceStoryReport {
  ...
  if (topGrowing && (topGrowing.pctVsLastWeek === null || topGrowing.pctVsLastWeek >= 5)) {
    if (topGrowing.pctVsLastWeek !== null) {
      highlights.push(
        `이번 주 제일 잘 큰 종목은 「${topGrowing.name}」예요. 지난주보다 볼륨이 약 ${topGrowing.pctVsLastWeek}% 올랐어요.`,
      );
    } else if (topGrowing.lastWeekVolume === 0 && topGrowing.thisWeekVolume > 0) {
      highlights.push(`이번 주 새로 쌓인 볼륨이 「${topGrowing.name}」에서 제일 눈에 띄어요.`);
    }
  }
  ...
  if (highlights.length === 0) {
    if (thisRoll.rowCount === 0) {
      highlights.push("이번 주 기록이 아직 없어요. 세트 하나만 남겨도 여기 숫자가 바로 채워져요.");
    } else {
      highlights.push("부위 밸런스는 무난한 편이에요. 다음 주엔 종목 하나만 중량·세트만 살짝 바꿔도 재밌어져요.");
    }
  }
  ...
  if (rows.length === 0) {
    ...
    summaryParts.push(`이번 주는 볼륨이 새로 쌓이기 시작했어요. 합이 ${thisV}예요.`);
  } else if (wow !== null) {
    if (wow >= 8) {
      summaryParts.push(`이번 주는 지난주보다 볼륨이 약 ${wow}% 올랐어요.`);
    } else if (wow <= -8) {
      summaryParts.push(`이번 주는 지난주보다 볼륨이 약 ${Math.abs(wow)}% 줄었어요. 델로드 타이밍일 수도 있으니 컨디션만 체크해 봐요.`);
    } else {
      summaryParts.push(`이번 주는 지난주랑 볼륨 비슷해요(약 ${wow > 0 ? "+" : ""}${wow}%).`);
    }
  } else {
    summaryParts.push("지난주 리듬 그대로 이번 주도 이어가 보면 돼요.");
  }

  if (legsWow !== null && legsWow >= 15 && legsThis > 200) {
    summaryParts.push("하체 쪽 볼륨이 특히 많이 올랐어요.");
  } else if (legsWow !== null && legsWow <= -15 && legsLast > 200) {
    summaryParts.push("하체 볼륨은 지난주보다 줄었어요. 다음엔 가볍게라도 패턴만 유지해 봐요.");
  }
